...
export interface CalculateOrderRequest {
  ...
  giftCardCode: string;
  customSurcharges: Array<CustomSurchargesDetails>;
  shippingOptions: ShippingOptionInfo;
  handlingFee: HandlingFeeInfo;
}

export default interface OrderEntry {
  id: string;
  subtotal?: number;
  total?: number;
  usdTotal?: number;
  ...
  files?: Array<OrderItemProductFile>;
}

interface OrderItemProductFile {
...
interface HandlingFeeInfo {
  ...
  value: number;
  description?: string;
  taxes: HandlingFeeTaxes;
}

interface HandlingFeeTaxes {
  name: string;
  value: number;
  total: number;
}
...
interface CustomSurchargesDetails {
  id?: string;
  value: number;
  type: string;
  description?: string;
...
interface SelectedOptions {
  ...
  type?: string;
  files?: Array<OrderItemOptionFile>;
}
